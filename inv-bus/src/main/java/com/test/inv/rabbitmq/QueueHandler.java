...
public class QueueHandler {
    ...
    public void listenerAutoAck(String str, Message message, Channel channel) {
        // TODO 如果手动ACK,消息会被监听消费,但是消息在队列中依旧存在,如果 未配置 acknowledge-mode 默认是会在消费完毕后自动ACK掉
        final long deliveryTag = message.getMessageProperties().getDeliveryTag();
        try {
            log.info("[listenerAutoAck 监听的消息] - [{}]", str);
    ...
    public void listenerManualAck(String str, Message message, Channel channel) {
        log.info("[listenerManualAck 监听的消息] - [{}]", str);
        try {
            // TODO 通知 MQ 消息已被成功消费,可以ACK了
